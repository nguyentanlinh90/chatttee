...
public class CouponUserGetCouponRequest extends CouponApiRequest {

    private String id;

    public CouponUserGetCouponRequest(String id, APIResponseListener listener) {
        super(true, CouponRequestTarget.COUPON_USER_GET_COUPON, listener);
        this.id = id;
    }

    @Override
    public void requestApi(CouponApiManager couponApiManager, Context context) {
        couponApiManager.getUserGetCoupon(
                String.format(requestTarget.toString(), id), getUserToken())
