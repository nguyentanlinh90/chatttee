...
public class RangeSeekBar<T extends Number> extends ImageView {
     ...
     */
    public static final int ACTIVE_COLOR = TCUtils.getColor(R.color.c_b2973f);
    public static final int DEFAULT_COLOR = TCUtils.getColor(R.color.c_8e8e93);
    /**
    ...
    private static final int LINE_HEIGHT_IN_DP = 4;
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.seek_bar_thumb;
        int thumbPressed = R.drawable.seek_bar_thumb;
        int thumbDisabled = R.drawable.seek_bar_thumb;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = TCUtils.dpToPx(context, 2);
        int defaultShadowXOffset = TCUtils.dpToPx(context, 0);
        int defaultShadowBlur = TCUtils.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = TCUtils.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = TCUtils.dpToPx(context, LINE_HEIGHT_IN_DP);
            activeColor = ACTIVE_COLOR;
            defaultColor = DEFAULT_COLOR;
            alwaysActive = false;
            showTextAboveThumbs = true;
            textAboveThumbsColor = TCUtils.getColor(R.color.c_b2973f);
            ...
            try {
                ...
                textAboveThumbsColor = a.getColor(R.styleable.RangeSeekBar_textAboveThumbsColor, TCUtils.getColor(R.color.c_b2973f));
                ...
                defaultColor = a.getColor(R.styleable.RangeSeekBar_defaultColor, DEFAULT_COLOR);
                ...
                if (normalDrawable != null) {
                    thumbImage = TCUtils.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = TCUtils.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbPressed);
                if (pressedDrawable != null) {
                    thumbPressedImage = TCUtils.drawableToBitmap(pressedDrawable);
                }
        ...
        textSize = TCUtils.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = TCUtils.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + TCUtils.dpToPx(context,
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : TCUtils.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = TCUtils.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
