...
public class VendorScreen extends TCCouponBaseFragment implements FavouriteAndShareListener, APIResponseListener {

    private static final String ID_VENDOR = "ID_VENDOR";
    private static final String VENDOR_DETAIL_MODEL = "VENDOR_DETAIL_MODEL";
    private static final String IS_GOTO_COUPON_TAB = "IS_GOTO_COUPON_TAB";
...
//    CircleIndicatorPager circle_page;
    ...
    private String id;
    private boolean isGotoCouponTab;
    ...
    public static VendorScreen getInstance(String idVendor) {
        VendorScreen screen = new VendorScreen();
        Bundle bundle = new Bundle();
        bundle.putSerializable(ID_VENDOR, idVendor);
        screen.setArguments(bundle);
        return screen;
    }

    public static VendorScreen getInstance(String idVendor, boolean isGotoCouponTab) {
        VendorScreen screen = new VendorScreen();
        Bundle bundle = new Bundle();
        bundle.putSerializable(ID_VENDOR, idVendor);
        bundle.putBoolean(IS_GOTO_COUPON_TAB, isGotoCouponTab);
        screen.setArguments(bundle);
        return screen;
    }

    /*public static VendorScreen getInstance(VendorDetailModel vendorDetailModel, boolean isGotoCouponTab) {
        VendorScreen screen = new VendorScreen();
        Bundle bundle = new Bundle();
    ...
    public void onBindView() {
        ...
        if (bundle != null) {
            id = bundle.getString(ID_VENDOR);
            detailModel = (VendorDetailModel) bundle.getSerializable(VENDOR_DETAIL_MODEL);
            isGotoCouponTab = bundle.getBoolean(IS_GOTO_COUPON_TAB);
        }
...
//        getReviewStatus();
    ...
    private void getData() {
        ...
        } else if (!TCUtils.isEmpty(id)) {
            requestApi(new ReviewShopGetVendorDetailRequest(id,  this));
    ...
    private void setupFragmentForViewPager() {
        if (isAdded()) {
            tv_get_deals_now.setVisibility(detailModel.isHaveCatalogueCoupon()?View.VISIBLE:View.GONE);
            adapter = new ViewPagerAdapter(getChildFragmentManager());
            adapter.addFragment(BuildConfig.IS_APP_USER ?
                    VendorDetailScreen.newInstance(detailModel) : VendorReviewScreen.newInstance(detailModel, false), "");
            //adapter.addFragment(VendorDetailTabReviewsScreen.newInstance(detailModel), "");
            ...
            if (isGotoCouponTab) {
                VPagerTabMenu.setCurrentItem(3);//set default selected page is Coupon Tab
                view_custom_tab_menu_vendor_tv_coupon.setSelected(true);
                tv_get_deals_now.setVisibility(View.GONE);
            } else {
            ...
            VPagerTabMenu.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
                ...
                public void onPageSelected(int position) {
                    ...
                    if(detailModel!=null){
                        if(detailModel.isHaveCatalogueCoupon()){
                            tv_get_deals_now.setVisibility(position==0?View.VISIBLE:View.GONE);
                        }
    ...
    private void fillData() {
       ...
       // collapsing_toolbar.setTitle(detailModel.getName());
        ...
        rating_bar.setRating(TCUtils.roundRating(detailModel.getRating()));
    ...
    public void updateLikeItemList(int position) {
        if (adapter != null) {
            if(tabPosition==1){
                ...
                VendorDetailScreen vendorDetailScreen = (VendorDetailScreen)
                        adapter.getFragment(VendorDetailScreen.class.getCanonicalName());
                if (vendorDetailScreen != null) {
                    vendorDetailScreen.updateItemList(position);
                }
            }
